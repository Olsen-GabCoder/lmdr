...
class PrivateChatViewModel @Inject constructor(
    ...
    val chatItems: StateFlow<List<ChatItem>> = _messages.map { addDateSeparators(it) }
    ...
    fun triggerTierUpgradeEffect(message: String) { viewModelScope.launch { _events.emit(ChatEvent.TierUpgrade(message)) } }
    private fun loadUsers() { firebaseAuth.currentUser?.uid?.let { uid -> userProfileRepository.getUserById(uid).onEach { _currentUser.value = it }.launchIn(viewModelScope) }; userProfileRepository.getUserById(targetUserId).onEach { _targetUser.value = it }.launchIn(viewModelScope) }
    @DrawableRes private fun getAffinityIcon(score: Int): Int = when { score >= TIER_3_THRESHOLD -> R.drawable.ic_heart_fire; score >= TIER_2_THRESHOLD -> R.drawable.ic_heart_red; score >= TIER_1_THRESHOLD -> R.drawable.ic_heart_red; else -> R.drawable.ic_heart_white }
    private fun addDateSeparators(messages: List<PrivateMessage>): List<ChatItem> { if (messages.isEmpty()) return emptyList(); val fullChatList = mutableListOf<ChatItem>(); val calendar = Calendar.getInstance(); val sortedMessages = messages.sortedBy { it.timestamp }; sortedMessages.forEachIndexed { index, message -> val messageDate = message.timestamp ?: return@forEachIndexed; val previousMessage = sortedMessages.getOrNull(index - 1); if (index == 0 || (previousMessage?.timestamp != null && !isSameDay(previousMessage.timestamp, messageDate, calendar))) { fullChatList.add(DateSeparatorItem(timestamp = messageDate)) }; fullChatList.add(MessageItem(message = message)) }; return fullChatList }
    private fun isSameDay(date1: Date, date2: Date, calendar: Calendar): Boolean { calendar.time = date1; val year1 = calendar.get(Calendar.YEAR); val day1 = calendar.get(Calendar.DAY_OF_YEAR); calendar.time = date2; val year2 = calendar.get(Calendar.YEAR); val day2 = calendar.get(Calendar.DAY_OF_YEAR); return year1 == year2 && day1 == day2 }
    fun formatDateLabel(date: Date): String { val calendar = Calendar.getInstance(); val today = calendar.time; calendar.add(Calendar.DAY_OF_YEAR, -1); val yesterday = calendar.time; val targetCalendar = Calendar.getInstance().apply { time = date }; return when { isSameDay(date, today, Calendar.getInstance()) -> "AUJOURD'HUI"; isSameDay(date, yesterday, Calendar.getInstance()) -> "HIER"; else -> { val format = if (Calendar.getInstance().get(Calendar.YEAR) == targetCalendar.get(Calendar.YEAR)) SimpleDateFormat("d MMMM", Locale.FRENCH) else SimpleDateFormat("d MMMM yyyy", Locale.FRENCH); format.format(date).uppercase(Locale.FRENCH) } } }
    private fun formatUserStatus(user: User?, isTyping: Boolean): String { if (isTyping) return "en train d'écrire..."; if (user?.isOnline == true) return "en ligne"; val lastSeenDate = user?.lastSeen ?: return ""; val diff = System.currentTimeMillis() - lastSeenDate.time; return when (val minutes = TimeUnit.MILLISECONDS.toMinutes(diff)) { in 0..0 -> "Dernière activité à l'instant"; in 1..59 -> "Dernière activité il y a $minutes min"; else -> when (val hours = TimeUnit.MILLISECONDS.toHours(diff)) { in 1..23 -> "Dernière activité il y a $hours h"; else -> if (TimeUnit.MILLISECONDS.toDays(diff) < 2) "Dernière activité hier" else "Dernière activité le ${SimpleDateFormat("dd/MM/yy", Locale.FRENCH).format(lastSeenDate)}" } } }
    private var typingTimeoutJob: Job? = null; private var isCurrentlyTyping = false; private val textInputFlow = MutableStateFlow("")
    fun onUserTyping(text: String) { textInputFlow.value = text }
    private fun observeTypingStatus() { textInputFlow.debounce(TYPING_DEBOUNCE_MS).onEach { typingTimeoutJob?.cancel(); val currentUserId = firebaseAuth.currentUser?.uid; val convId = conversationId.value; if (currentUserId == null || convId == null) return@onEach; if (it.isNotBlank() && !isCurrentlyTyping) { isCurrentlyTyping = true; privateChatRepository.updateTypingStatus(convId, currentUserId, true) }; if (it.isBlank() && isCurrentlyTyping) setTypingStatus(false) else if (it.isNotBlank()) { typingTimeoutJob = viewModelScope.launch { delay(TYPING_TIMEOUT_MS); setTypingStatus(false) } } }.launchIn(viewModelScope) }
    private fun setTypingStatus(isTyping: Boolean) { typingTimeoutJob?.cancel(); val currentUserId = firebaseAuth.currentUser?.uid; val convId = conversationId.value; if (currentUserId != null && convId != null && isCurrentlyTyping != isTyping) { isCurrentlyTyping = isTyping; viewModelScope.launch { privateChatRepository.updateTypingStatus(convId, currentUserId, isTyping) } } else if (isCurrentlyTyping != isTyping) { isCurrentlyTyping = isTyping } }
    fun setChatActive(isActive: Boolean) { _isChatVisible.value = isActive; val convId = conversationId.value; val userId = firebaseAuth.currentUser?.uid; if (convId != null && userId != null) { viewModelScope.launch { privateChatRepository.updateUserActiveStatus(convId, userId, isActive) } }; if (isActive && convId != null) { markConversationAsRead(convId) } }
    ...
    fun addOrUpdateReaction(messageId: String, emoji: String) { val convId = conversationId.value; val uid = firebaseAuth.currentUser?.uid; if (convId == null || uid == null) return; viewModelScope.launch { privateChatRepository.addOrUpdateReaction(convId, messageId, uid, emoji) } }
    private fun markConversationAsRead(conversationId: String) { viewModelScope.launch { privateChatRepository.markConversationAsRead(conversationId) } }
